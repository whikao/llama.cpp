            ...
            if (raw_q4_0_staging) {
                ...
                if (tensor->nb[2] != 0 &&
                    ...
                    if (dbg_v102_host_count < 16) {
                        ...
                        GGML_LOG_INFO(
                            "DBG_V106_HOST_REF: dev=%s tensor=%s expert=%zu ct=0 valid_rows=%d "
                            "copy_off=%zu copy_size=%zu nb01=%zu nb02=%zu raw_bytes=%zu "
...
struct ggml_hexagon_opqueue {
    ...
    void pop(htp_opbatch_rsp rsp, dspqueue_buffer dbuf) {
        ...
        // the same cache-coherent DSP -> Host queue path as id/status/n_ops.
        if (opt_mmid_raw_q4_0 &&
            rsp.dbg_magic == HTP_OPBATCH_DEBUG_RAW_Q4_0_MAGIC) {
            static int dbg_v104_host_count = 0;
            if (dbg_v104_host_count < 64) {
                GGML_LOG_INFO(
                    "DBG_V106_DSP_RETURN: dev=%s batch=%u op=%u expert=%u ct=%u ith=%u valid_rows=%u "
                    "src_off=%u raw_fnv=%08x tile_fnv=%08x\n",
                    shm_buf->sess->c_name(),
                    rsp.id,
                    rsp.dbg_op_index,
                    rsp.dbg_expert,
                ...
                union { uint32_t u; float f; } o0, o1, o2, o3;
                o0.u = rsp.dbg_out0;
                o1.u = rsp.dbg_out1;
                o2.u = rsp.dbg_out2;
                o3.u = rsp.dbg_out3;
                GGML_LOG_INFO(
                    "DBG_V108_DOT_OUT: dev=%s batch=%u op=%u expert=%u ct=%u "
                    "out_bits=%08x,%08x,%08x,%08x out_f=%g,%g,%g,%g\n",
                    shm_buf->sess->c_name(),
                    rsp.id,
                    rsp.dbg_op_index,
                    rsp.dbg_expert,
                ...
                GGML_LOG_INFO(
                    "DBG_V114_INTDOT: dev=%s batch=%u op=%u expert=%u ct=%u "
                    "k_boundary=%u tile=%u k0=%u k1=%u "
                    "hvx_tile=%g scalar_ref=%g hvx_bits=%08x ref_bits=%08x "
                    "hvx_int_dot=%d scalar_int_dot=%d "
                    "q4_scale_fp16=%04x q8_scale_fp16=%04x\n",
                    shm_buf->sess->c_name(),
                    rsp.id,
                    rsp.dbg_op_index,
                    rsp.dbg_expert,
