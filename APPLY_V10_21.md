# Hexagon HMX raw-Q4_0 MMID v10.43 phone overlay
...
- v10.43 returns to the accepted v10.41 source; none of the rejected v10.42
  register-scale code is present. v10.42 remained numerically correct but fell
  to 1.38 t/s for 8 tokens and 1.76 t/s for 32 tokens, consistent with extra
  HVX register pressure/spills. v10.43 leaves the scale in the original
  scratch vector. For every full 32-row Q4_0 tile it masks the upper 64 padding
  bytes to zero and performs one aligned 128-byte store, replacing v10.41's
  predicated 64-byte scale store plus separate 64-byte memset. The resulting
  640-byte tiled block, dot inputs, accumulation order and fast-path guard are
  byte-identical to v10.41. This also benefits full-row generic conversion
  without adding a live register across the dot kernel.
- v10.41 adds a narrowly guarded fused down path. For the verified raw-Q4_0
  ...
  the existing Q4_0/Q8_0 dot and the fp32 output accumulator remains in HVX
  registers across all 24 K blocks. This avoids materializing and rereading
  the full 15,360-byte tiled intermediate per row tile. The converter's gather
  addresses and packed bytes and the dot's integer/fp16/fp32 equations are
  unchanged and execute in the same K order. Other shapes, duplicate expert
  mappings, partial row tiles, and `MMID_DEBUG=1` retain the exact v10.40 path.
...
- v10.38 corrects the v10.37 probe target.  The real decode down operand is
  ...
  `hvx_mv_id_raw_q4_0` worker previously assumed.  This also means the v10.36
  ring implementation was never executed by the measured generation path.
  Its 1.66 versus 1.74 t/s result cannot be attributed to that code and is now
  treated as phone temperature/frequency or run variance.  v10.37 restored the
  old layout and measured 1.75 t/s, consistent with that correction.
- This remains one focused diagnostic build, not a speculative optimization.
  It instruments only raw-Q4_0 `ffn_down_exps` with K=768, N=2048 and eight
  routed rows. `DBG_V138_HVX_RAW_DOWN_PROFILE` reports summed worker, DMA,
  raw-to-tiled conversion and HVX dot times plus per-op wall time, tile count
  and residual time. Gate/up, prompt HMX math and tensor values are unchanged.
  Timer calls add diagnostic overhead, so use this version to choose the next
  code change rather than as a final speed benchmark.
...
tar -xzf /sdcard/Download/llama-hexagon-hmx-raw-mmid-v10.43.tar.gz -C .
git diff --check
git status --short
```
...
First run one deterministic 8-token correctness check because v10.43 changes
the scale/tail store. Keep all debug profilers disabled so
...
compare against the v10.41 control of 14.769 seconds / 2.17 t/s. Do not use
v10.42 as the performance baseline.
